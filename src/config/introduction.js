export default {
  defaultIntro: {
    ...
    <p>4.而对于面包屑导航和页面说明则定义了Title和Introduction组件来实现，也都在src/layout目录下</p>
    <p>5.这两个组件本身定义了模板内容，现在你看到的文字就是模板内容里的，如果你到别的页面，那么根据页面传递的内容，现在你看到的这些内容将会被覆盖成新的内容（即对应页面的文字说明），而这个内容的传递是通过内置的slot组件实现的</p>
    <p>6.然后在Layout组件当中这两个组件也拼在了里面，而你看到的这些说明内容都是从src/config/introduction.js里读取的，然后根据路由读取对应路由下的文字说明以及面包屑导航地址</p>
  ...
  testComponent: {
    index: "组件使用测试",
    content: `该页面就是简单的组件引用和通信示例，其实现主要说明如下：
    <p>1.浅黄色背景的内容只是单纯的通过import导入了组件并使用</p>
    <p>2.浅绿色背景的两个按钮分别实现了接收子组件的值和向子组件传值</p>
    <p>3.接收子组件的值是通过父组件设置props属性，里面存放可能接收的子组件的传值，然后子组件通过将值放入对应的属性里进行传值</p>
    <p>4.向子组件传值是通过父组件通过$emit设置一个事件和对应发送的值，然后在子组件监听该事件，然后获取对应的数据</p>
    `
  },
  testVuex: {
    index: "vuex测试",
